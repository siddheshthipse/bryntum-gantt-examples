...
class ExamplesApp {
  constructor() {
    ...
    const version = VersionHelper.getVersion(me.product.name),
      ...
      store = me.examplesStore = new Store({
        data: examples,
        fields: ['folder', 'rootFolder', 'fullFolder', 'group', 'title', 'overlay', 'version', 'build', 'since', 'offline', 'ie', 'edge', 'id', 'updated', 'globalUrl', {
          name: 'tab',
          defaultValue: 'js'
        }],
        groupers: [{
    ...
    EventHelper.on({
      scroll: {
        handler() {
          ...
          jumpTo.value = (topElement === null || topElement === void 0 || (_topElement$dataset = topElement.dataset) === null || _topElement$dataset === void 0 ? void 0 : _topElement$dataset.group) ?? null;
        },
        element: browserEl,
        throttled: 250
      },
      ...
      items: {
        ...
        filterField: {
          ...
          filterFunction(record, value) {
            ...
            return value === null || value === void 0 ? void 0 : value.toLowerCase().split(' ').every(word => ['title', 'version', 'folder'].some(param => {
              var _record$param;
              return (_record$param = record[param]) === null || _record$param === void 0 ? void 0 : _record$param.toLowerCase().includes(word);
            }));
          },
  ...
  filterDemos(framework) {
    ...
    this.jumpTo.items = [{
      id: 'top',
      text: 'Top'
    }].concat(this.exampleStore.groupRecords.map(r => ({
      id: r.id,
  ...
  scrollToLocationHash() {
    ...
    if (hash) {
      ...
      if (match = /-frameworks-(\w+)/.exec(hash)) {
        button = buttons[match[1]];
      } else {
        button = buttons.js;
      }
      if (button) {
        button.pressed = true;
        me.filterDemos(button.ref);
      }
  ...
  getDomConfig() {
    ...
    me.examplesStore.records.forEach(example => {
      if (example.isSpecialRow) {
        ...
        let html = group;
        if (groupPaths[group]) {
          const tip = me.isOnline ? 'Path in distribution after download' : 'Click to view files in the folder if local web server is configured to allow directory listing',
        ...
        const isSF = example.folder === 'salesforce',
    ...
    Widget.attachTooltip(me.examplesContainerEl, {
      ...
      getHtml: async ({
        ...
        if (!me.currentTipLoadPromiseByURL[url]) {
          try {
            const requestPromise = me.currentTipLoadPromiseByURL[url] = AjaxHelper.get(url, {
                parseJson: true
              }),
      ...
      click(event) {
        ...
        if (el.dataset.external === 'true') {
          ...
          new Popup({
            ...
            html: me.buildTip + `The demo can be found in distribution folder: <div class="tip-folder"><i class="b-fa b-fa-folder-open"></i><b>` + (!me.isOnline ? `<a href="${el.dataset.linkUrl}">${el.dataset.linkText}</a>` : el.dataset.linkText) + '</b></div>',
            closeAction: 'destroy',
            width: el.getBoundingClientRect().width,
            anchor: true,
            scrollAction: 'realign'
  ...
  onThumbError(e) {
    var _e$target;
    if ((_e$target = e.target) !== null && _e$target !== void 0 && (_e$target = _e$target.src) !== null && _e$target !== void 0 && _e$target.includes('thumb')) {

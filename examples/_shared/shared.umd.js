...
window.bryntum.product = {
  name: 'gantt',
  fullName: 'Bryntum Gantt',
  onlineId: 'gantt-vanilla',
  appId: '9df03cbdc00b455de8bfe58d831e6fd76cc8881e'
};
/* global RC */
// Load product config
...
const errorListener = errorEvent => earlyErrorEvent = errorEvent,
  ...
  shouldShow = (item, filterValue) => {
    var _item$cls;
    return !item.isMenuItem || ((_item$cls = item.cls) === null || _item$cls === void 0 ? void 0 : _item$cls['b-docs-category']) || item.text.toLowerCase().includes(filterValue);
  };
...
const hintKey = 'preventhints-' + document.location.href,
  ...
  browserPaths = ['/examples/', '/grid/', '/scheduler/', '/schedulerpro/', '/gantt/', '/calendar/', '/taskboard/'],
  themes = {
    stockholm: 'Stockholm',
    classic: 'Classic',
...
class Shared extends Localizable(Events()) {
  ...
  constructor() {
    ...
    if (isBryntumCom && !VersionHelper.isTestEnv) {
      console.log(`%c TIP! %cYou can access the widget used in the demo on the console using %cwindow.${me.product.ref || me.product.name}`, 'color: orange; font-weight: bold', 'color : initial', 'font-family: monospace');
    }
  }
            ...
            async beforeShow() {
              if (!this.items.length) {
                ...
                for (const meta of Object.values(bryntum.usedClasses)) {
                  var _publicClasses$name;
                  const {
                      name
                    } = meta,
                    // Docs app needs "/" as delimiter
                    fullName = (_publicClasses$name = publicClasses[name]) === null || _publicClasses$name === void 0 ? void 0 : _publicClasses$name.replaceAll('.', '/');
                  if (meta.type && fullName && (
                  // check if the showcases the class
                  // skip unrelated classes ..to make the menu less busy
                  !featuredClassesRegExp || (meta.isFeatured = fullName.match(featuredClassesRegExp)) || !hideClassesRegExp || !fullName.match(hideClassesRegExp))) {
                ...
                // 4) by name
                classMeta.sort((a, b) => {
                  const aTypeWeight = typeWeight[a.type],
                    bTypeWeight = typeWeight[b.type];
                  return aTypeWeight > bTypeWeight ? 1 : aTypeWeight < bTypeWeight ? -1 : a.weight > b.weight ? -1 : a.weight < b.weight ? 1 : a.name > b.name ? 1 : -1;
                });
                let category;
  ...
  async initHints() {
    ...
    if (!hints || me.preventHints || ((_me$toolTips = me.toolTips) === null || _me$toolTips === void 0 ? void 0 : _me$toolTips.length) > 0) {
      return;
    }
    me.toolTips = [];
    const missingTargets = Object.keys(hints).filter(key => !(key && DomHelper.down(document.body, key))),
  ...
  cleanupHints() {
    ...
    if (!me.preventHints) {
      ...
      if (me.toolTips) {
        me.toolTips.forEach(tip => {
          var _tip$hide;
          return (_tip$hide = tip.hide) === null || _tip$hide === void 0 ? void 0 : _tip$hide.call(tip).then(() => tip.destroy());
        });
  ...
  async loadHints(test = false) {
    ...
    if (!me.preventHints) {
      try {
        me.hints = (await AjaxHelper.get('meta/hints.json', {
          parseJson: true
        })).parsedJson;
  ...
  async showCodeEditor(noAnimation) {
    ...
    await codeEditor.expand(noAnimation ? {
      animation: null
    } : undefined);
    codeEditor.focus();
    return codeEditor;
...
class CodeEditor extends Panel {
  ...
  static #_ = _defineProperty(this, "configurable", {
    textContent: false,
    scrollable: null,
    collapsible: {
      type: 'overlay',
  ...
  async updateJS() {
    ...
    try {
      ...
      imports === null || imports === void 0 || imports.forEach(imp => {
      ...
      for (const project of projects) {
        var _project$destroy;
        (_project$destroy = project.destroy) === null || _project$destroy === void 0 || _project$destroy.call(project);
      }
...
window.addEventListener('message', ({
  ...
  if (data) {
    ...
    if ((_message = message) !== null && _message !== void 0 && _message.style) {
      const container = document.getElementById('container');

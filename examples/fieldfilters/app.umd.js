  ...
  try {
    savedFilters = JSON.parse(savedFiltersJSON, function (key, value) {
      if (key === 'value' && this.type === 'date') {
        ...
        return Array.isArray(value) ? value.map(durationStr => new Duration(durationStr)) : new Duration(value);
      }
      return value;
    });
  } catch (e) {
...
const getFieldFilterPickerConfig = filter => filter.id === 'permanent-filter-1' ? {
  propertyLocked: true,
  operatorLocked: true
} : undefined;
...
const gantt = new Gantt({
  ...
  columns: [{
    ...
    renderer: ({
      ...
      var _record$color;
      return (_record$color = record.color) === null || _record$color === void 0 ? void 0 : _record$color.name;
    }
  }]
});
      ...
      items: {
        enableAllCheckbox: {
          ...
          listeners: {
            change: ({
              checked
            }) => checked ? app.widgetMap.filterPickers.activateAll() : app.widgetMap.filterPickers.deactivateAll()
          }
        },
        filterPickers: {

...
import { BryntumDemoHeader, BryntumGantt, BryntumSplitter, BryntumTreeGrid } from '@bryntum/gantt-react';
import { type Model, type Store, ProjectModel, DependencyModel, StringHelper, TaskModel, type SchedulerEventModel } from '@bryntum/gantt';
import { ganttConfig, treeGridConfig } from './AppConfig';
import './App.scss';

function App() {
    const
        ganttRef    = useRef<BryntumGantt>(null),
        treeGridRef = useRef<BryntumTreeGrid>(null);

    useEffect(() => {
        const
            gantt    = ganttRef.current!.instance,
            treeGrid = treeGridRef.current!.instance;
    ...
    return (
        <>
            ...
            <BryntumDemoHeader />
            <div className="b-wrapper b-widget">
                <BryntumGantt
                    ref={ganttRef}
                    {...ganttConfig}
                />

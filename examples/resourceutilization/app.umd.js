...
({
  ...
  return ((_origin$event = origin.event) === null || _origin$event === void 0 ? void 0 : _origin$event.projectTask) || 'No Project';
}];
const resourceUtilization = new ResourceUtilization({
  ...
  listeners: {
    ...
    cellClick({
      ...
      const task = record.isAssignmentModel ? record.event : (_record$key = record.key) !== null && _record$key !== void 0 && _record$key.isProjectTask ? record.key : null;
      if (task) {
  ...
  columns: [{
    ...
    renderer({
      ...
      var _record$key2, _record$key3;
      // Unwrap record to its origin - resource or assignment
      record = grid.resolveRecordToOrigin(record);
      if ((_record$key2 = record.key) !== null && _record$key2 !== void 0 && _record$key2.isResourceModel) {
        record = record.key;
      }
      ...
      // If that's an assignment row
        ...
        var _record$event;
        return StringHelper.encodeHtml((_record$event = record.event) === null || _record$event === void 0 ? void 0 : _record$event.name);
      }

      // Otherwise record represents a group
      // so record.key might have: resource, event or city
      return ((_record$key3 = record.key) === null || _record$key3 === void 0 ? void 0 : _record$key3.name) || record.key;
    }
  }, {
    ...
    items: [{
      ...
      items: [{
        ...
        onAction() {
          ...
          ({
            origin
          }) => origin.isResourceModel ? Store.StopBranch : origin.resource]);
        }
      }, {
        text: 'Default',

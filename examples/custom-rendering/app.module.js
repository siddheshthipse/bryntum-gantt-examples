import { TaskModel, InstancePlugin, GridFeatureManager, DateHelper, DomHelper, EventHelper, StringHelper, Gantt } from '../../build/gantt.module.js';
...
const
    getResourceImage = resource => gantt.resourceImagePath + (resource.image !== false ? StringHelper.encodeHtml(resource.name.toLowerCase() + '.jpg') : 'none.png'),
    gantt            = new Gantt({
        project : {
            taskModelClass : Task,
            autoLoad       : true,
        ...
        columns : [
            ...
            { type : 'percentdone', showCircle : true, width : 100, align : 'center' },
            { type : 'duration' },
            {
                type    : 'action',
                width   : 45,
        ...
        onDateChange({ value }) {
            this.scrollToDate(value, { animate : 300, block : 'start' });
        },

        onShowAvatarsChange({ checked }) {
        ...
        onRowHeightInput({ value }) {
            this.rowHeight = value;
        },

        onBorderRadiusInput({ value }) {
            this.element.style.setProperty('--task-border-radius', `${value}px`);
        },

        onGridColumnLinesChange({ value }) {
            this.columnLines = value;
        },

        onScheduleTickLinesChange({ value }) {
            this.features.columnLines.disabled = !value;
        },

        onNonworkingTimeChange({ value }) {

...
const ALL_ROWS = 0;
const ADDED_ROWS = 1;
const UPDATED_ROWS = 2;
const ADDED_AND_UPDATED_ROWS = 3;
const REMOVED_ROWS = 4;
const META_DATA = 5;
...
abstract class SyncHandler
{
    ...
    public function handleAdded($request, &$response)
    {
        ...
        if (sizeof($added)) {
            $response['rows'] = @$response['rows'] ? array_merge((Array)$response['rows'], $added) : $added;
        }
    ...
    public function handleUpdated($request, &$response)
    {
        ...
        if (sizeof($updated)) {
            $response['rows'] = @$response['rows'] ? array_merge((Array)$response['rows'], $updated) : $updated;
        }
    ...
    public function handleRemoved($request, &$response)
    {
        ...
        if (sizeof($removed)) {
            $response['removed'] = @$response['removed'] ? array_merge((Array)$response['removed'], $removed) : $removed;
        }

...
```text
The Gantt is sized by its predefined minHeight, likely this is not intended.
...
This guide uses Gantt in the snippets, but the same principles applies to Scheduler, Scheduler Pro, Gantt,
...
You can let the Bryntum Gantt decides its own height by using the `autoHeight` property.
